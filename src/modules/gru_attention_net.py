...
class GruAttentionNet(torch.nn.Module):
    def __init__(self, dim_embeddings, hidden_size, output_size, hidden_layer, bidirectional):
        ...
        '''
        super(GruAttentionNet, self).__init__()
        ...
        # first gru used to extract feature from embeding word of the context and option
        self.gru1 = torch.nn.GRU(dim_embeddings, self.hidden_size, hidden_layer, bidirectional=bidirectional)
        # second gru used to extract feature from attentioned option
        self.gru2 = torch.nn.GRU(self.feature*3, self.hidden_size, hidden_layer, bidirectional=bidirectional)
        ...
        self.mlp = torch.nn.Sequential(
            torch.nn.Linear(self.feature*2, self.hidden_size),
    ...
    def forward(self, context, context_lens, options, option_lens):
        ...
        context, _ = self.gru1(context.transpose(1, 0))
        ...
        for i, option in enumerate(options.transpose(1, 0)):
            ...
            option, _ = self.gru1(self.pad(option.transpose(1, 0)))
            ...
            attention_cont = self.softmax(attention_cont).unsqueeze(1) # (batch, 1, opt_seq)
